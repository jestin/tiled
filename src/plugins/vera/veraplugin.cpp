...
bool VeraPlugin::write(const Map *map, const QString &fileName, Options options)
{
    ...
    if (enableViews) {
        ...
        for (const Layer *layer : map->objectGroups()) {
            ...
            for (const MapObject *object : objectLayer->objects()) {
                ...
                if (viewCount > 7) {
                    Tiled::ERROR(QLatin1String("VERA plugin: Can't export more than 8 views."),
    ...
    while (const Layer *layer = iterator.next()) {
                ...
                for (int x = 0; x < tileLayer->width(); ++x) {
                    ...
                    if (const Tile *tile = cell.tile()) {
                        ...
                        if (cell.flippedAntiDiagonally()) {
                            Tiled::WARNING(QStringLiteral("VERA plugin: Rotated tiles are not supported."),
        ...
        case Layer::ObjectGroupType: {
            ...
            for (const MapObject *object : qAsConst(objects)) {
                ...
                if (const Tile *tile = object->cell().tile()) {
                    ...
                    Tiled::WARNING(QStringLiteral("VERA plugin: Ignoring non-tile object %1 without type.").arg(object->id()),
                                   Tiled::JumpToObject { object });
                }
            }
            break;
        ...
        case Layer::ImageLayerType:
            ...
            Tiled::WARNING(QStringLiteral("VERA plugin: Ignoring image layer \"%1\" (not currently supported).").arg(layer->name()),
